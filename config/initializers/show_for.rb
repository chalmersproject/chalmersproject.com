...
ShowFor.setup do |config|
  ...
  # Skip blank attributes instead of generating with a default message. Default
  # is false.
  ...
  config.default_collection_proc = ->(value) do
    "<li>#{ERB::Util.h(value)}</li>".html_safe # rubocop:disable Rails/OutputSafety
  end
  ...
  # in which the association responds to is used to retrieve the association
  # labels.
  # config.association_methods = [ :name, :title, :to_s ]

  # If you want to wrap the text inside a label (e.g. to append a semicolon),
  # specify label_proc - it will be automatically called, passing in the label
  # text.
